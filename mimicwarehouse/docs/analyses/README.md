...
| — | Data-quality profile: `runs/<run_id>/qc_report.md` + `qc_tables.csv` / `qc_checks.csv` (EP-44; the `kind: qc` run of `mwh build --tier <t> --tag qc`) | exploratory (data-quality profile) | **pending promotion at EP-53** — written already suppressed (`disclose.suppress`, k = 11) and passing `mwh disclose check` on every file (`test_ep44` asserts it on the fixture); the run ids of the dev and full runs are in the EP-44 completion note; no results copied here |
| — | Measurement process: `runs/<run_id>/measurement_process.md` + the six `mp_*.csv` tables (EP-45; the `kind: qc` run named `measurement` of `mwh build --tier <t> --tag measurement`) | exploratory (measurement process) | **pending promotion at EP-53** (EP-72 renders the views over the same tables) — written already suppressed (`disclose.suppress`, k = 11; the item summary + absence summary as one frame) and passing `mwh disclose check` on every file (`test_ep45` asserts it on the fixture); the run ids of the dev and full runs are in the EP-45 completion note; no results copied here |
