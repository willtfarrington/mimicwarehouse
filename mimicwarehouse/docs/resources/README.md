...
| [concepts.md](concepts.md) | mimic-code concept inventory (65 `concepts_duckdb` files at the EP-8 pin): execution order, dependencies, per-file sha256, status on DuckDB 1.5.5 (all execute on every tier; the failure list is empty) and the deviations table — the five EP-38 patches with their upstream PR references, semantics and demo count effects | EP-37, EP-38 |
