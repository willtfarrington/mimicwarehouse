...
| `concepts/` — vendored MIT-LCP/mimic-code at the D-19 pin (`vendor_info()`, `VENDOR.json`); all 65 `concepts_duckdb` files execute cleanly on DuckDB 1.5.5 (EP-33 D2). **EP-37:** `inventory.py` (scan → generated `concepts.yaml` + `dag/specs/concepts.yaml` + the docs table; `python -m mimicwarehouse.concepts.inventory`), `runner.py` (one `python` step `concept.<group>.<name>` per concept → `lake/derived/<tier>/mimiciv_derived/<name>/part-0.parquet`, single ZSTD file; `meta.concept_versions` under a `kind: build` run with `kind: concept` benchmark lines), `pins.py` (count-pins through `safe_query`; `tests/ep/pins/concepts_demo.json` committed, dev pins under `runs/pins/`). **EP-38:** `patches/` — `patches.yaml` registry + one `<concept>.sql` full replacement per patched concept (SIRS wbc guard, MCHC/CRP `valueuom`, Charlson C4A, APS-III equidistant arms; all `ported-unmerged` upstream PRs); the runner validates the registry per build, refuses a patch written against another upstream commit, prefers the patch and records `patch_id` in `status.json` / `meta.concept_versions`; `KNOWN_FAILURES` stayed empty (full tier 65/65) | EP-8, EP-37, EP-38 | `poe vendor-mimic-code` · `mwh build --tier t --tag concepts [--keep-going] [--select concept.<g>.<n> --with-deps]` · `mwh runs benchmarks --kind concept --tier t` · `python -m mimicwarehouse.concepts.patches` | `test_ep08.py` · `test_ep37.py` · `test_ep38.py` |
...
| `dag/` — `spec.py` (YAML DAG; `load_dag()` merges every `specs/*.yaml` — `stage.yaml` + EP-37's `concepts.yaml` — with one shared `catalog` step; `Step.status_key`; `--with-deps` closure), `runner.py` (`STEP_HANDLERS` stage/catalog/python, generic status-key skip, `--keep-going` with `failed`/`blocked` statuses, build lock with pid + create_time identity, snapshot ids), `jobs.py` (detached ⏱ supervisor, state files, logs), `benchmarks.py` (ledger lines, `summarize`, Markdown renderer), `snapshot.py` (layer snapshot ids incl. per-tier layers, manifest stats, `layer_lines`) | EP-19, EP-28, EP-29, EP-32, EP-33, EP-37 | `mwh build --tier t [--select a,b] [--tag t] [--force] [--with-deps] [--keep-going] [--break-lock] [--dry-run] [--background --job N]` · `mwh jobs [--job N] [--tail N]` | `test_ep19.py` · `test_ep20.py` · `test_ep28.py` · `test_ep32.py` · `test_ep37.py` |
| `catalog/` — `build.py` (per-tier `.duckdb`: dims materialized, subject-keyed tables as views, `meta.*` population, `CATALOG_EXTENSIONS` (+ `CatalogExtensionContext`, EP-37), published via `publish.swap_file`), `discover.py` (EP-37: every complete `lake/derived|marts/<tier>/<schema>/<table>` as a view, every `lake/meta/<tier>/*.parquet` as a `meta.*` table), `connect.py` (`open_catalog`, READ_ONLY + hardening + version/dev-bucket checks), `profile.py` (`meta.profile` step; `write_meta_parquet`), `dictionary.py` (`DATA-DICTIONARY.md`), `cli.py` (`mwh catalog`, `mwh sql`) | EP-21, EP-29, EP-30, EP-33, EP-37 | `mwh catalog info \| dictionary [--tier t] [--out]` · `mwh sql "<stmt>" [--tier t] [--k n] [--format table\|csv\|json] [--describe]` | `test_ep21.py` · `test_ep29.py` · `test_ep30.py` · `test_ep37.py` |
...
| `run.py` — the provenance run ledger (EP-35): `run.start(name, tier=, kind=, params=)` context manager → `runs/<run_id>/manifest.json` (`RunManifest`: git sha + dirty, `uv.lock` sha256, versions, params, snapshot ids, refs, `sql/`, `tables/`, `figures/`, attrition, audit ids, warnings, wall/RSS/disk, doctor block, error) + one `runs/ledger.jsonl` line; `run.bench` → the benchmark ledger; `runs_db_views` → the `runs.duckdb` views; `reproduction_block(run_id)`; prose in [docs/methods/provenance.md](docs/methods/provenance.md). **Seeds + resources (EP-36):** `derive_seed(protocol_id, stage, salt)` (sha256 → 32-bit, pure), `rng` / `spawn_rngs` / `random_state` / `seed_everything`, `r.seed(stage)` → `manifest.seeds`; `ResourceLog` daemon sampler (0.5 s: wall, CPU, peak working set + source, disk delta, per-pid GPU via optional `pynvml`) → `manifest.resources` (`ResourceUsage`), `ResourceLog.measure` + `bench(usage=)`; policy in [docs/methods/determinism.md](docs/methods/determinism.md) | EP-35, EP-36 | via `mwh runs` · `mwh tracer` (runs inside `run.start`) | `test_ep35.py` · `test_ep36.py` |
...
uv run mwh build --tier dev --tag concepts                          # EP-37: the 65 mimic-code concepts -> lake\derived\dev\mimiciv_derived\<name>\part-0.parquet, meta.concept_versions, catalog (dev: ~25 s)
uv run mwh build --tier dev --select concept.sepsis.sepsis3 --with-deps --force   # rebuild one concept; incomplete ancestors run, complete ones skip (--keep-going: record a failure, block its dependents, continue)
uv run python -m mimicwarehouse.concepts.patches   # EP-38: refresh + validate concepts/patches/patches.yaml (a patched concept is rebuilt with --select ... --force; patch_id lands in meta.concept_versions)
...
uv run mwh runs list --last 5                # EP-35 provenance run ledger (runs\ledger.jsonl), newest first; --tier/--kind/--json; `mwh runs show <run_id>` prints a manifest (hashes, counts, params, paths - never rows)
uv run mwh runs benchmarks --format md       # the staging benchmark table from runs\benchmarks.jsonl (EP-32; `--out` splices it into docs/analyses/00-staging-benchmark.md); `--kind concept --tier dev` = the EP-37 per-concept lines
uv run mwh sql "SELECT concept, rows FROM meta.concept_versions ORDER BY 1" --tier dev   # EP-37: one row per attempted concept (status/error recorded, never hidden)
