...
| `codesets/` — the code-set registry + ICD-9 <-> ICD-10 GEM utility (EP-40): `spec.py` (pydantic `CodeSet`: slug id, semver, `kind` in `icd_dx | icd_px | itemid | drug | atc | loinc | hcpcs`, members by system — dual `icd9` + `icd10` lists for every diagnosis / procedure set, `{code, match: exact|prefix, group}` entries, drug-name lists, ATC classes — normalised codes and the canonical-JSON `def_hash`), `registry.py` (the 20 packaged seeds under `defs/` + `codesets.lock.json`: the `(id, version)` immutability rule → `CodeSetFrozenError`; dictionary expansion against `d_icd_diagnoses` / `d_icd_procedures` / `meta.itemids` / `d_hcpcs`; the `codesets.compile` DAG step → `meta.codesets` + `meta.codeset_members`; `validate` through `safe_query`; the `register_codesets` catalog extension; [docs/methods/codesets.md](docs/methods/codesets.md) renderer), `gem.py` (CMS 2018 GEM fetch into `ext/vocab/gem/2018/` + `source.yaml`, parser, `forward` / `backward`, the `codesets.gem` step → `meta.gem_i9_to_i10` / `meta.gem_i10_to_i9`, the `.gem-review.md` author aid), `cli.py`. **EP-172 (2026-09-16):** the owner-supervised GEM review adjudication — `t2dm` all rejected (stays `t2dm@1.0.0`; manifestation codes of combination entries are rejected as a rule), `sepsis_explicit@1.1.0` locked (9 cross-era twins accepted, 5 rejected; 21 pairs in the lock); the decision ledger is in the brief's completion note | EP-40, EP-172 | `mwh codeset list \| show <ref> \| lock [--check] \| validate <ref> --tier t \| compile [--tier t] [ref …] \| expand <ref> --via-gem \| gem fetch \| gem status` · `mwh build --tier t --tag codesets` · `python -m mimicwarehouse.codesets` | `test_ep40.py` |
| `phenotypes/` — the phenotype engine (EP-41): `spec.py` (pydantic `Phenotype`: slug id, semver, `grain` in `subject | hadm | icustay`, a `criteria` tree of `all` / `any` / `not` over leaves — `diagnosis` / `procedure` / `medication` / `lab` / `microbiology` / `concept` / `temporal` — an `onset` rule, `outputs`, `what_it_does_not_claim`; `def_hash` = canonical JSON of grain + criteria + onset + the referenced code-set hashes), `registry.py` (`defs/*.yaml` + `phenotypes.lock.json`: the `(id, version)` immutability rule → `PhenotypeFrozenError`, reference resolution against the EP-40 registry, `validate`), `compiler.py` (the deterministic CTE chain with the code sets and lab conversions inlined; `tests/ep/golden/<ref>.sql`), `runner.py` (the `phenotypes.compile` DAG step → `lake/derived/<tier>/phenotypes/<id>@<version>/part-0.parquet` + `meta.phenotype_versions`, one `kind: phenotype` run per phenotype; `register_phenotypes` — `mimiciv_derived.phenotype_<id>` = the latest built version + the `_hadm` companion; `summarize` / `summary` through `safe_query`; the [docs/methods/phenotypes.md](docs/methods/phenotypes.md) renderer), `cli.py`; first definition `t2dm@1.0.0`. **EP-42:** top-level `parameters` (`$name` placeholders resolved into the criteria), the concept leaf's `window` (hours from the key table's anchor, `timesem.sql_hours_since`) and `evidence` (typed per-unit columns: `first` / `last` / `min` / `max`, `default`, `levels`), **concept pins** (`ConceptPin`: the executed-SQL sha256 of every vendored concept a leaf reads, from the inventory + patch registry, hashed into `def_hash`, refused at materialisation when the tier's concept build differs; `meta.phenotype_versions.concept_refs`; the pinned concept steps are `depends_on` of `phenotypes.compile`), the icustay `_hadm` companion (admissions with ≥ 1 ICU stay, any-stay flag, `n_stays`), keep-going over the selected phenotypes, `distribution` / `agreement` / `prevalence_report` → `runs/<run_id>/phenotype_prevalence.md` (claim type exploratory; sidecar pending EP-43); definitions `sepsis3@1.0.0`, `kdigo_aki@1.0.0` (`min_stage`, `window_hours`), `sepsis_explicit@1.0.0`. **EP-172:** `sepsis_explicit@1.1.0` (the 1.0.0 tree over the reviewed code set; golden `tests/ep/golden/sepsis_explicit@1.1.0.sql`; built on dev + full, the session view follows it; 5 pairs in the lock) | EP-41, EP-42, EP-172 | `mwh phenotype list \| show <ref> [--sql] \| validate <ref> \| lock [--check] \| compile [ref …] [--tier t] [--force] [--dry-run] [--background --job N] \| summary <ref …> --tier t [--k n] [--report \| --out DIR] [--no-agreement]` · `mwh build --tier t --tag phenotypes` · `python -m mimicwarehouse.phenotypes` | `test_ep41.py` · `test_ep42.py` |
| `cohort/` — the cohort spec + registry (EP-46; DESIGN §9): `spec.py` (pydantic `CohortSpec`: slug id, semver, `grain` from `timesem.GRAINS` — placeholders refused — an `index_event` {`rule` of the grain's registry entry \| `phenotype_onset` \| `concept_time`}, ordered `inclusion` / `exclusion` lists of labelled criteria — `age` / `demographic` / `codeset` / `phenotype` / `concept` / `los` / `data_availability` / `prior_admissions` / `custom_sql` (flagged `custom`) — `observation_window` `[start_h, end_h)`, `washout`, `follow_up` over `timesem.CENSORING_RULES` (the `dod` horizon rule; `discharge_alive` competing), `era_filter` ⊆ `timesem.ERAS`, the `degeneracy_probe` columns, `references`; absolute dates and age bounds in the cap band refused; `def_hash` = canonical JSON of everything but the documentation fields with the referenced code-set / phenotype hashes inlined; `to_yaml` / `from_yaml` / `save_yaml`, the JSON schema for EP-62's form), `registry.py` (the two packaged seeds under `specs/` + `cohorts.lock.json`: the `(id, version)` immutability rule → `CohortSpecFrozenError`, reference resolution against the EP-40 / EP-41 registries, `validate`, `save_spec`, the `cohorts.specs` DAG step → `meta.cohort_specs`, the `register_cohorts` catalog extension, the [docs/methods/cohorts.md](docs/methods/cohorts.md) renderer), `probe.py` (through `safe_query`: references against `meta.codesets` / `meta.phenotype_versions`, the level-degeneracy probe — zero-event / all-event levels of the probe columns against the outcome over the index population, k-suppressed; the EP-31 policy's spec-level half), `cli.py`; seeds `first_icu_adults@1.0.0` (the tracer bullet's cohort) and `hf_admissions@1.0.0` (`heart_failure@1.0.0` by reference). **Compiler + materialisation (EP-47):** `compiler.py` (`compile_spec` / `compile_entry` → `CompiledCohort`: one deterministic CTE chain `base` → `idx` → [`era`] → `crit_NN_<label>` per criterion in spec order → [`washout`] → `cohort` — the grain keys, `index_time`, `era_index`, `age_at_index` / `age_capped`, `obs_start` / `obs_end`, `follow_up_end` / `censor_reason`, `custom_flag`, total `ORDER BY`, no non-deterministic function; `custom_sql` as a semi-joined `custom_NN` CTE, flagged; the `attrition_sql` `UNION ALL` of `count(*)` / `count(DISTINCT subject_id)` per step; `sql_sha256`; golden `tests/ep/golden/first_icu_adults@1.0.0.sql`), `build.py` (the `cohorts.build` python step, tag `marts` — relations bound on the build connection with the resolved hashes or refused with the remedy; one `kind: cohort` run per spec → `lake/marts/<tier>/cohorts/<id>@<version>/{cohort.parquet, attrition.parquet, spec.yaml, manifest.json}` via `publish.swap_dir`, per-tier status `cohorts.<ref>`, the marts snapshot id, a `kind: mart` benchmark line, the **suppressed** chain in the run manifest; byte-identical rebuilds, same-hash skip, moved-definition refusal unless `--force`; `attrition()` → the chain after `disclose.suppress(mode="chain")` on both count columns; `register_marts` → `marts.cohort_<id>_v<major>` (latest patch per major) + the `marts.cohorts` registry table, `rows` / `n_subjects` blanked below k); the probe runs over the compiled cohort once built. **Attrition diagram (EP-48):** `attrition.py` — the STROBE-style renderers over the suppressed frame (`render_mermaid`: one node per step, a dotted `Excluded at <step>` node per non-zero drop, a footer with id@version / tier / run / def_hash / k, entity-code escaping, palette colours; `render_altair` / `to_vegalite` / `to_png`: the Vega-Lite funnel with the EP-5 theme merged at serialisation, PNG through `vl-convert-python` — core since EP-48; `render_markdown`: claim type `exploratory (cohort description)`, the retrospective sentence, the table with drops as `n = X` text cells, the diagram, the spec's non-claims, the EP-35 reproduction block), the **pair guard** (a step whose written `n_units - n_subjects` would fall in `(0, k)` has its subjects total and the two drops beside it withheld — `suppressed` — because `disclose.check` refuses such nested totals), withheld drops beside banded totals drawn as the rounded released difference (`~20`), the `cohort` step without an exclusion by construction, and `save_attrition` → `runs/<run_id>/figures/attrition.{mmd,vl.json,png,csv,md}` staged under `tmp/`, every file through `disclose.check` before anything is written, recorded in the build run's `figures`; `suppress_attrition` gained the `dropped_*_small` markers | EP-46, EP-47, EP-48 | `mwh cohort list \| show <ref> [--yaml] \| validate <ref> [--tier t] [--k n] \| schema [--out PATH] \| lock [--check] \| build [ref …] --tier t [--force] [--dry-run] [--background --job N] [--json] \| attrition <ref \| run_id> --tier t [--k n] [--format table\|mermaid\|altair\|all] [--out DIR] [--title T] [--direction TD\|LR\|…] [--json]` · `mwh build --tier t --tag cohorts` (the registry index) · `mwh build --tier t --tag marts` (the builds) · `python -m mimicwarehouse.cohort` | `test_ep46.py` · `test_ep47.py` · `test_ep48.py` |
| `timeline.py` — the event-aligned timeline API (EP-49; DESIGN §7 note, [docs/methods/timelines.md](docs/methods/timelines.md)): the anchor registry (`Anchor` / `ANCHORS`: `hosp_admit` · `hosp_discharge` · `icu_in` · `icu_out` · `first_culture` · `first_antibiotic` · `suspected_infection` · `vent_start` · `deterioration` + the factories `med_start(codeset, source)` / `procedure(itemids)` / `vent_start(kinds)` / `phenotype_onset(id@version)` / `custom_anchor`; `anchor_sql(anchor, grain)` → `(grain keys, anchor_time)` for every unit, NULL where absent, `selector` first / last / each, the join-key rule across grains), the event-source presets (`labs` / `vitals` / `inputs` / `outputs` / `meds` / `procedures` / `micro` / `transfers` / `custom_source`; `code` / `valueuom` cut to 64 chars), `align` / `align_sql` → `(keys, anchor_time, event_time, hours_since_anchor, code, value, valueuom, source_table)` over `[start, end)` windows with `clip_to_stay`, `window_join`, `asof_join` (DuckDB `ASOF JOIN`, backward / forward, tolerance), `event_at` (the last value at or before the anchor within a tolerance), `hourly_bins` / `daily_bins` (`timesem.sql_hour_bin`; `fill=True` adds empty bins), `population_summary` (per code × bin: `n_units`, pooled mean, per-unit quartiles, extremes — through the `safe.SUPPRESSOR` seam; event counts deliberately not released, D-33 addendum), `to_mart` (Parquet via `publish.swap_file`); every builder returns SQL text and a lazy relation. **Owner-only:** `stay_events(stay_id, sources, conn=)` needs a connection opened as `open_catalog(tier, role="owner")` — the opener now stamps the role as the session variable `mwh_role`; any other connection raises `PermissionError` — and writes one `row_view:` audit line per call (D-32 addendum). **Benchmark:** `run_benchmark` / `mwh timeline bench` — five curated labs aligned to `icu_in` over `[-6, 48)` h → hourly bins → the released summary inside a `kind: bench` run with a `kind: query` ledger line `timeline_labs_icu_in_48h`, the frame under `tables/` and four gated exports with sidecars under `runs/<run_id>/exports/` (`.parquet`, `.md` twin, `population_band.png` + `.csv`) | EP-49 | `mwh timeline anchors [--json]` · `mwh timeline bench --tier t [--background --job N] [--json]` · `python -m mimicwarehouse.timeline` | `test_ep49.py` |
| `spine.py` — the events spine (EP-50; DESIGN §10, [docs/methods/spine.md](docs/methods/spine.md)): the registry `SPINE_SOURCES` — one `SpineSource` per core table (13: `patients` → `MEDS_BIRTH` (synthetic, `age_capped` flag) / `MEDS_DEATH`, `admissions`, `transfers`, `icustays`, `diagnoses_icd` (at `dischtime`), `procedures_icd`, `labevents`, `microbiologyevents`, `prescriptions`, `emar`, `inputevents` (+ `INPUT_RATE`), `outputevents`, `procedureevents`; raw `chartevents` excluded in v1) with its SQL projection into the MEDS 0.4 column order (`subject_id` int64, `time` µs, `code`, `numeric_value` float32, `text_value` + `hadm_id`, `stay_id`, `source_table`), the `PREFIX//segment//segment` code grammar with `NONE` segments and cuts that keep every `code` / `text_value` within the 64-char safe-query bound; the `spine.<source>` DAG steps (`dag/specs/spine.yaml`, tag `spine`; `build_source` → `lake/derived/<tier>/spine/source=<s>/subject_bucket=NN/part-0.parquet`, one sorted single-file `COPY` per bucket, `swap_dir`, a manifest line per file, per-tier status `spine.<s>`, a `kind: mart` ledger line; resumable, `--select spine.<s>`) and `spine.union` (`build_union` → `meta.spine_codes`, k-suppressed at build time, raw counts under `lake/meta/<tier>/raw/`; `validate` → `meta.spine_validation`; the `mimiciv_derived.spine` status entry); `validate(tier)` — MEDS core schema per file (pyarrow types, no `meds` dependency), `time` non-null except `MEDS_BIRTH`, subjects in `patients`, `code` / `text_value` ≤ 64 chars, sorted files, the static denied-column walk (`check_source`: `comments` / `text` / `note` + the contract's `free_text` flags); `register_spine` (a `CATALOG_EXTENSIONS` entry: the view over every complete source + comments; EP-37's walker skips the bucketed `spine/` dir); the [docs/methods/spine.md](docs/methods/spine.md) renderer | EP-50 | `mwh build --tier t --tag spine` (full: `--background --job spine-full`) · `mwh spine sources [--json]` · `mwh spine validate --tier t [--json] [--no-write]` · `python -m mimicwarehouse.spine` | `test_ep50.py` |
...
| `qc/` — data-quality profiling (EP-44; DESIGN §15): `profile.py` (the thresholds document `qc/thresholds.yaml` — pydantic `Thresholds`: per-check `warn_above` / `fail_above` / `warn_below` / `fail_below`, the declared dictionary-coded columns, the `timestamp_order` / `store_lag` / `event_window` rules — validated against the contract; `dictionary_coded_columns` = FK-into-a-dimension + the declared list, never an identifier or free-text column; one SQL family per contract table generated from the contract: `profile_table` (quantiles `p01` / `p50` / `p99` via `approx_quantile`, top-k values of the dictionary-coded columns; `null_pct` / `n_distinct_approx` / `min` / `max` **reused from the EP-29 profile**, a missing or stale profile refused with the remedy) and the named checks `check_pk_unique` / `check_natural_key_dupes` / `check_fk_orphans` / `check_null_share` / `check_ts_order` / `check_ts_store_lag` / `check_event_window` / `check_units` (`unit_consistency` + `implausible_values` over the EP-39 catalogue, conversions inlined per itemid) / `check_age_cap` / `check_era_coverage` — aggregates only, `n_affected` counts; the `qc.profile.<schema>.<table>` steps (JSON slices under `lake/meta/<tier>/raw/qc/`, measured with `ResourceLog`) and `qc.checks` (thresholds evaluated, the four tables assembled inside a `run.start(kind="qc")` run with one `run.bench(kind="query")` line per table, `n_affected` in `0 < n < k` blanked with `n_affected_suppressed` + its `value`, top-k through `disclose.suppress` per column, raw twins under `raw/`) → `meta.qc_tables` / `meta.qc_columns` / `meta.qc_topk` / `meta.qc_checks`; `register_qc` (a `CATALOG_EXTENSIONS` entry: comments); `spec_document` renders `dag/specs/qc.yaml`), `report.py` (`qc.report`: `runs/<run_id>/qc_report.md` + `qc_tables.csv` / `qc_checks.csv` — claim type exploratory (data-quality profile), the retrospective sentence, table summary, checks by status, warnings and failures, unit variants, implausible shares, timestamp ordering, non-claims, the EP-35 reproduction block; every frame through `disclose.suppress`, every integer through `fmt_int`, `<11` cells — passes `mwh disclose check` on every file; the [docs/methods/qc.md](docs/methods/qc.md) renderer), `cli.py` (`mwh qc status`). **Measurement process (EP-45):** `measurement.py` — `MeasurementParams` (a python step's `params`: `itemids` ⊆ the EP-39 catalogue, `hours` 168, `days` 14, `first_window_hours` 24, `min_stays` 50, `sparse_share` 0.05), the SQL builders over `timesem` (population = ICU stays with a valid window + a patients row; distinct measurement occasions per source, labs attached by subject + window; `[start, end)` hour / day bins with `n_stays_at_risk`; unit × era cells; the safe-query-shaped presence statement), `compute_hourly` / `compute_structural` / `compute_presence` (raw slices under `lake/meta/<tier>/raw/measurement/`), `assemble` (`disclose.suppress` per table — the item summary + absence summary as one frame with the additive-identity pass, statistics and flags blanked beside a hidden count, the presence cross-contrast rule, rate ratios with a Wald 95 % CI from released counts via statsmodels) → `meta.mp_item_hourly` / `mp_item_daily` / `mp_item_summary` / `mp_structural` / `mp_absence_summary` / `mp_presence_outcome` inside one `kind: qc` run (`measurement`), `runs/<run_id>/measurement_process.md` + six CSVs (claim type exploratory (measurement process); passes `mwh disclose check`), `register_measurement`; `dag/specs/measurement.yaml` (steps `measurement.hourly` / `.structural` / `.presence` / `.report`, tag `measurement`); `cli.py` gains `mwh qc measurement`; prose twin [docs/methods/measurement-process.md](docs/methods/measurement-process.md) | EP-44, EP-45 | `mwh build --tier t --tag qc` (full: `--background --job qc-full`) · `mwh qc status --tier t [--show fail\|warn\|all\|none] [--json]` (exit 1 when any check failed) · `mwh build --tier t --tag measurement` (full: `--background --job measurement-full`) · `mwh qc measurement --tier t [--top N] [--json]` · `mwh runs benchmarks --tier t --kind query` · `python -m mimicwarehouse.qc` | `test_ep44.py` · `test_ep45.py` |
...
| [docs/methods/timelines.md](docs/methods/timelines.md) | the event-aligned timeline API (EP-49): the anchor table (generated), the join-key rule, window / clip / as-of semantics, bins and the released population summary, the alignment benchmark, the owner-only single-stay path, examples |
| [docs/methods/spine.md](docs/methods/spine.md) | the events spine (EP-50): the MEDS mapping and code grammar tables (generated), exclusions and the free-text rule, time semantics (synthetic birth, dod horizon, diagnoses at discharge), build / layout / resume, the seven validation checks, reading it through `mwh sql`, sizes |
...
poethepoet, pre-commit) · `ui` (Streamlit 1.61, VegaFusion, Plotly — Streamlit pins
`pyarrow<25`, so `[tool.uv] conflicts` keeps `ui` apart from `gpu` and `text`;
`vl-convert-python` moved from `ui` to the core set at EP-48 for the attrition PNG) ·
...
## Quick start (`mwh` as of EP-50: `doctor` · `paths` · `guard` · `verify` · `schema` · `inventory` · `fixtures` · `canary` · `build` · `jobs` · `catalog` · `sql` · `demo` · `runs` · `tracer` · `units` · `codeset` · `phenotype` · `disclose` · `qc` · `cohort` · `timeline` · `spine`; `app` lands in P4)
...
uv run mwh build --tier dev --tag qc                                  # EP-44: one qc.profile step per table + qc.checks + qc.report + catalog -> meta.qc_tables / qc_columns / qc_topk / qc_checks (needs the tier's meta.profile: `--select meta.profile,catalog` first if it is missing or stale); full: `--background --job qc-full`, `mwh jobs --job qc-full --tail 20`
uv run mwh qc status --tier dev --show all                            # pass / warn / fail counts per check + the flagged rows through safe_query (n_affected below k reads `<11`); exit 1 when any check failed; --json
uv run mwh runs list --kind qc                                        # the kind: qc runs; `mwh runs show <run_id>` = the run whose folder holds runs\<run_id>\qc_report.md + qc_tables.csv + qc_checks.csv (already suppressed; `mwh disclose check` passes)
uv run mwh runs benchmarks --tier dev --kind query                    # per-table QC profile wall / rows / peak RSS (one kind: query line per table, EP-44)
uv run mwh sql "SELECT check_id, status, count(*) AS n FROM meta.qc_checks GROUP BY 1, 2" --tier dev   # meta.qc_* are registry tables: read them like meta.columns (counts of checks below k are still suppressed - `mwh qc status` counts in Python instead)
uv run python -m mimicwarehouse.qc                                    # regenerate dag/specs/qc.yaml from the contract + qc/thresholds.yaml and the generated blocks of docs/methods/qc.md
uv run mwh build --tier dev --tag measurement                         # EP-45: measurement.hourly + .structural + .presence (raw slices) + measurement.report (one kind: qc run -> meta.mp_item_hourly / mp_item_daily / mp_item_summary / mp_structural / mp_absence_summary / mp_presence_outcome, k-suppressed at build time, + runs\<run_id>\measurement_process.md + CSVs) + catalog; full: `--background --job measurement-full`, `mwh jobs --job measurement-full --tail 20`
uv run mwh qc measurement --tier dev --top 10                         # per curated itemid: stays measured, share measured in the first 24 h, median occasions per stay-day, interval p10/median/p90, cells in use / sparse / structural + the largest structural (unit x era) cells, through safe_query (`<11` = suppressed at build time); --json
uv run mwh build --tier dev --select measurement.report,catalog       # re-suppress and re-render the six tables + the report from the existing slices (no rescan; e.g. after a k change)
uv run mwh cohort list                                                # EP-46: the packaged cohort specs (id@version, grain, index event, criteria, references, locked, def_hash); `show first_icu_adults@1.0.0 [--yaml]` prints the card (or the canonical YAML); no data access
uv run mwh cohort validate hf_admissions@1.0.0                        # static checks: grain + index rule, reference kinds (a codeset criterion needs a billed ICD set), tables, custom_sql flagged (problems exit 1; no data access)
uv run mwh cohort validate hf_admissions@1.0.0 --tier dev             # + through safe_query: every referenced code set / phenotype compiled on the tier with the resolved hash, and the level-degeneracy probe (zero-event / all-event levels of gender / admission_type / first_careunit / anchor_year_group against the outcome over the index population; k = 11 withholds small levels); --k n only on fixture / demo; --json
uv run mwh cohort schema --out cohort-spec.schema.json                # the JSON schema of a spec (pydantic + the exactly-one-kind-key rule) - what EP-62's Cohort Builder form renders
uv run mwh cohort lock --check                                        # every packaged (id, version) pair is recorded in cohorts.lock.json (exit 1 otherwise; `lock` records new pairs - an edited released version is refused with exit 3, bump `version`); `--specs DIR` for a study's cohorts/ folder
uv run mwh build --tier dev --tag cohorts                             # cohorts.specs + catalog: meta.cohort_specs (the registry index: id, version, def_hash, grain, index event, criterion counts, custom flag, refs, outcome, locked, path); `mwh sql "SELECT cohort_id, version, def_hash, grain FROM meta.cohort_specs" --tier dev` reads it (a meta.* registry read - no count column needed)
uv run mwh cohort build first_icu_adults@1.0.0 --tier dev             # EP-47: compile the spec to one deterministic CTE chain and materialise it under lake/marts/dev/cohorts/first_icu_adults@1.0.0/ (cohort.parquet, attrition.parquet, spec.yaml, manifest.json) inside a kind: cohort run, rebuild the catalog (marts.cohort_first_icu_adults_v1 + the marts.cohorts registry) and re-run the degeneracy probe over the compiled cohort; `--dry-run` prints the SQL; `--force` rebuilds (byte-identical from the same inputs); full tier: `--tier full --background --job ep47-cohort-full` then `mwh jobs --job ep47-cohort-full`
uv run mwh cohort attrition first_icu_adults@1.0.0 --tier dev         # the attrition chain (units / subjects left and dropped per step) after disclose chain-mode suppression (small totals `<11`, small drops withheld and their neighbours banded `~1,000`); raw counts stay in the mart; `mwh sql "SELECT cohort_id, version, tier, rows FROM marts.cohorts ORDER BY 1,2,3" --tier dev` lists the builds (a registry read - no count column needed)
uv run mwh timeline anchors                                                # EP-49: the anchor registry (hosp_admit, icu_in, first_culture, vent_start, deterioration, ...) and the factories; `--json`
uv run mwh timeline bench --tier dev                                       # EP-49: five curated labs aligned to icu_in over [-6, 48) h -> hourly bins -> the k-suppressed population summary, inside a kind: bench run (kind: query ledger line timeline_labs_icu_in_48h) with four gated exports + sidecars under runs\<run_id>\exports\; full tier: `--tier full --background --job ep49-timeline-bench` then `mwh jobs --job ep49-timeline-bench`; `mwh runs benchmarks --tier full --kind query` reads the line
uv run mwh build --tier dev --tag spine                                    # EP-50: the events spine - 13 spine.<source> steps -> lake\derived\dev\spine\source=<s>\subject_bucket=NN\ (sorted, MEDS 0.4 column order), spine.union -> meta.spine_codes (k-suppressed at build) + meta.spine_validation, then the catalog registers mimiciv_derived.spine; `--select spine.labevents` rebuilds one source; full tier: `--tier full --background --job spine-full` (EP-54 verifies)
uv run mwh spine sources                                                   # EP-50: the source registry and code grammar (no data is read); `--json`
uv run mwh spine validate --tier dev                                       # EP-50: the seven MEDS-conformance / governance checks over the built spine (exit 1 on a failure; `--no-write` skips the meta.spine_validation row)
uv run mwh sql "SELECT source_table, count(*) AS n FROM mimiciv_derived.spine GROUP BY 1 ORDER BY 1" --tier dev   # EP-50: reading the spine is an ordinary subject-keyed safe-query read; code / text_value stay within 64 chars so they can be group keys
uv run mwh cohort attrition first_icu_adults@1.0.0 --tier dev --format all   # EP-48: the STROBE-style attrition diagram - writes runs\<run_id>\figures\attrition.mmd (Mermaid) + .vl.json / .png (Altair funnel, vl-convert) + .csv (the suppressed table) + .md (claim type + retrospective statement + table + diagram + reproduction block), each through `disclose.check` first (refused = nothing written, exit 1); `--format mermaid | altair` print to stdout, `--out DIR`, `--title`, `--direction LR`
uv run mwh disclose check docs/methods/cohorts.md --allow-text meaning --allow-text type --allow-text default --allow-text fields --allow-text remarks --allow-text predicate --allow-text definition   # the methods page with its fixture-tier diagram (section 8) passes once its prose-table columns are admitted as labels
...
│                               cohort/ (EP-46: spec, registry, probe, cli; + specs/ and cohorts.lock.json),
│                               timeline (EP-49: anchors, alignment, bins, the owner-only stay_events, the benchmark),
│                               spine (EP-50: the MEDS-shaped events spine - registry, DAG steps, validate, the catalog view)
