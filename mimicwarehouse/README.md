...
| `run.py` — the provenance run ledger (EP-35): `run.start(name, tier=, kind=, params=)` context manager → `runs/<run_id>/manifest.json` (`RunManifest`: git sha + dirty, `uv.lock` sha256, versions, params, snapshot ids, refs, `sql/`, `tables/`, `figures/`, attrition, audit ids, warnings, wall/RSS/disk, doctor block, error) + one `runs/ledger.jsonl` line; `run.bench` → the benchmark ledger; `runs_db_views` → the `runs.duckdb` views; `reproduction_block(run_id)`; prose in [docs/methods/provenance.md](docs/methods/provenance.md) | EP-35 | via `mwh runs` · `mwh tracer` (runs inside `run.start`) | `test_ep35.py` |
