...
date, the MIMIC-IV caveats paragraph, the pending-EP-43 sidecar note), then one section
...
Until ``mwh disclose check`` ships (EP-43) the generated file is committed with the
header line saying its ``.disclosure.json`` sidecar is pending; EP-43 lists this file in
its acceptance.
"""
...
def render_dictionary(con: duckdb.DuckDBPyConnection) -> tuple[str, DictionaryResult]:
    ...
    lines: list[str] = [
        ...
        "> Disclosure sidecar pending EP-43 (`mwh disclose check` does not exist yet); "
