...
instead); ``summary`` prints the prevalence through ``safe_query`` (k-suppressed,
audited). Errors follow the EP-33 canon (:func:`mimicwarehouse.console.fail`): a frozen
``(id, version)`` — phenotype or code set — refuses with ``EXIT_REFUSED`` (3), usage /
environment errors exit 2, a failed compile or a validation with problems exits 1.
...
phenotype_app = typer.Typer(
    name="phenotype",
    help=(
        "Phenotype engine (EP-41): list | show | validate | lock | compile | summary. "
        "Phenotypes are versioned YAML (id@version, def_hash pinned to the code sets they "
        "reference) compiled to deterministic SQL and materialised per tier; sessions read "
        "mimiciv_derived.phenotype_<id> as aggregates."
    ),
...
def show_command(
    ...
    if json_output:
        ...
        if sql:
            ...
            payload["sql"] = compile_phenotype(p, registry.codesets, resolved=entry.resolved).sql
        emit_json(payload)
        return
    ...
    if sql:
        ...
        console.print(
            escape(compile_phenotype(p, registry.codesets, resolved=entry.resolved).sql),
            highlight=False,
        )
...
def compile_command(
    ...
    version bump) phenotype or code set is refused before anything runs."""
...
def summary_command(
    ctx: typer.Context,
    ref: Annotated[str, typer.Argument(help="Phenotype as <id>@<version> (the latest built).")],
    tier: TierOption = None,
    k: Annotated[
        int | None,
    ...
    """Prevalence of a built phenotype through safe_query: n_units, n_positive and share
    overall and by era (hadm_era) — k-suppressed, audited, aggregates only."""
    ...
    try:
        with safe_cli_errors(prefix):
            summary = phen_runner.summarize(ref, tier=resolved_tier, settings=settings, k=k)
    except PhenotypeError as exc:
        fail(prefix, str(exc), code=EXIT_USAGE)
    if json_output:
