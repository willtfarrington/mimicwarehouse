...
(
    ...
    SELECT 'pharmacy' AS tbl,           17847567 AS row_count UNION ALL -- mwh-guard: allow (row count, not an id)
    SELECT 'poe' AS tbl,                52212109 AS row_count UNION ALL
    SELECT 'poe_detail' AS tbl,         8504982 AS row_count UNION ALL
    SELECT 'prescriptions' AS tbl,      20292611 AS row_count UNION ALL -- mwh-guard: allow (row count, not an id)
    ...
    SELECT 'inputevents' AS tbl,        10953713 AS row_count UNION ALL -- mwh-guard: allow (row count, not an id)
