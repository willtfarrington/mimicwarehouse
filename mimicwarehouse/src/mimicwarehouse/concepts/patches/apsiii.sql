-- mimicwarehouse concept patch `apsiii-equidistant-arms` (EP-38, 2026-09-05).
-- Full replacement of mimic-iv/concepts_duckdb/score/apsiii.sql at MIT-LCP/mimic-code commit
...
-- Upstream reference: https://github.com/MIT-LCP/mimic-code/pull/2137
-- Change:
--   the six 'values are equidistant' arms (resp_rate, hematocrit, wbc, sodium, albumin,
--   glucose) compare the max arm to the min arm instead of to itself. Reachable only
--   when the two distances are equal, so the scores are unchanged; ported for intent.
