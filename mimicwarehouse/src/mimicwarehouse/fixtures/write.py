...
#: Write-side formats from the one CSV dialect (EP-169, retro SCH-1); names kept for callers.
TIMESTAMP_FORMAT = WRITE_TIMESTAMP_FORMAT
...
#: Manifest keys recorded for provenance only (which environment / contract text produced the
#: bytes): drift comparisons ignore them, so a comment-only contract edit or a numpy/polars
#: upgrade that moves no CSV byte does not fail the suite (EP-169, retro SCH-2/FXT-2). The
#: load-relevant pin is ``contract_schema_hash`` (``Contract.structural_hash()``).
...
def _manifest_payload(
    ...
    return {
        ...
        # provenance (MANIFEST_PROVENANCE_KEYS): the environment that produced the bytes -
        # numpy/polars deliberately unpinned (Risk 3); a lock bump that moves bytes is a
        # regeneration (retro FXT-2)
...
def render_manifest(
    ...
    payload = _manifest_payload(entries, spec, contract_hash, contract_schema_hash)
...
def render_readme(spec: FixtureSpec, tables: Sequence[str] | Mapping[str, Sequence[str]]) -> bytes:
    ...
    lines = [
        ...
        f"`subject_id` >= {spec.first_subject_id:_}, `hadm_id` >= {spec.first_hadm_id:_},",
        f"`stay_id` >= {spec.first_stay_id:_}, event/row ids >= {spec.first_event_id:_},",
        f"`caregiver_id` >= {spec.first_caregiver_id:_} - pairwise disjoint, so a wrong-key join",
...
def write_fixture(
    ...
    *,
    ...
    manifest_path.write_bytes(render_manifest(entries, spec, contract_hash, contract_schema_hash))
