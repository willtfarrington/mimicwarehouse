...
| Re-transpile mimic-code `concepts/` → DuckDB locally with upstream `mimic_utils` (sqlglot) instead of waiting for the upstream regeneration bot — parked by EP-8 (2026-08-17); re-parked unchanged by EP-37 (2026-09-05: all 65 vendored files execute on 1.5.5 through the runner, so the trigger is EP-38's count-pin verification, not executability) | EP-38 finds `concepts_duckdb/` lagging a fix we need (upstream regeneration PR #2157 open at the pin), or the PR stays open through P4 | sqlglot version drift vs upstream's; transpiled output must be diffed against the vendored tree and recorded as patches | v2 CONC-1 |
