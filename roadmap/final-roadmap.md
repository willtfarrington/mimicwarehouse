...
| Re-transpile mimic-code `concepts/` → DuckDB locally with upstream `mimic_utils` (sqlglot) instead of waiting for the upstream regeneration bot — parked by EP-8 (2026-08-17); re-parked by EP-37 (2026-09-05): all 65 vendored files execute on 1.5.5, so regeneration is only worth it for a lagging *fix* | EP-38 finds `concepts_duckdb/` lagging a fix we need, or the upstream regeneration PR (#2157, open at the pin) stays open through P4 | sqlglot version drift vs upstream's; transpiled output must be diffed against the vendored tree and recorded as patches | v2 CONC-1 |
| Upstream our concept patches as mimic-code PRs once their semantics are validated on the full tier — parked by EP-38 (2026-09-05): the five ports in `concepts/patches/patches.yaml` (SIRS wbc guard #2146, lab `valueuom` #2141, Charlson C4A #2142, APS-III equidistant arms #2137) follow *open* upstream PRs, so the first contribution is a review/+1 on those threads, not a new PR; a `local` patch (none yet) would need its own | P4 re-plan (EP-54) confirms the demo/dev/full counts stable and the upstream PRs still open; a `status: local` registry entry appears | contributor process/time; the upstream regeneration bot (#2157) must re-transpile `concepts_duckdb` after a merge, so `applies_to_upstream_commit` and the re-vendor (`poe vendor-mimic-code`) move together | v2 CONC-2 |
