...
| Alternative bucket schemes (256 hash buckets, or fewer buckets for small tables) instead of the flat `subject_id % 100` — parked by EP-18 (2026-08-29); v1 fixes 100 buckets for every partitioned table (DESIGN §5/§21) | EP-28's file-count/Defender measurement shows NTFS or scanner overhead dominating the stage | the bucket function is baked into every partition path, the dev-tier filter (D-18) and `loader/paths.py`; changing it means a full restage and a catalog/view rebuild (EP-33 re-plan decision, never a quiet edit) | v2 LOAD-2 |
