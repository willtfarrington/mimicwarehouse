...
**Notation used in briefs** (added at EP-166, D-43 item 13 / ledger CMP-5). Briefs were written before P0/P1a shipped and use planning-era shorthand; **this table overrides brief text**, and re-plans update the table, not each brief:

| Brief says | It means |
|---|---|
| `%MWH_DATA_ROOT%\x` / `$env:MWH_DATA_ROOT` | `get_settings().layout[...]` / `mwh paths --json` (default `C:\mimicdata`). **No `MWH_DATA_ROOT` env var is set on this machine** — never write `$env:MWH_DATA_ROOT` into a PowerShell command line: it expands to empty and targets the drive root. |
| `roadmap_check.py` | `mimicwarehouse/scripts/roadmap_check.py`, invoked as `uv run poe roadmap-check [--strict] [--json]` ≡ `mwh verify --roadmap`. `--strict` exits 0 since EP-164 (Risk 14). |
| `DEV_BUCKETS` (a constant) | `get_settings().dev_buckets` (EP-3; default `[0..4]`) — the only bucket source, no separate constant (D-43 item 14). |
| `uv run mwh …` | ≡ `uv run --group dev mwh …` (`default-groups = ["dev"]`). |
| "doctor green" | exit 0 = no `fail` row; the `antivirus` **warn is expected** on this host (D-38/D-42). |
| test naming | file `tests/ep/test_ep<NN>.py` (zero-padded), marker `ep_<n>` (unpadded — `poe test -m ep_08` selects nothing). |
| integers in tracked Markdown / completion notes | thousands-separated via `mimicwarehouse.inventory.fmt_int` (guard G4 refuses bare 8-digit tokens starting 1–3); `--json` output keeps raw ints and is **never pasted into tracked files** (EP-10 rule, ledger FC-16). Never `mwh-guard: allow` on data-derived numbers. The full committed-text canon (file names, run folders, pragma policy, DOI rule) is [`../mimicwarehouse/docs/committed-text.md`](../mimicwarehouse/docs/committed-text.md) (EP-33). |
| `mimicwarehouse.paths` · `swap_catalog` · `catalog.cli.EXIT_REFUSED` · `inventory.open_connection` (as the opener) · `mwh runs bench` · `safe.owner_rows` | planning-era / P2 names consolidated at EP-33 (D-45): `mimicwarehouse.publish` (`swap_dir` / `swap_file`) · `console.EXIT_*` · `engine.open_duckdb(profile)` · `mwh runs benchmarks` · no such API (EP-49 defines its own owner gate). P3 briefs carry the correction in their `EP-33 amendment` block; the ledger is `retro-p2.md` § Renames. |
| a `_fail`/`typer.Exit` error print, a `--json` emitter, a JSONL append | `console.fail` (stderr, `mwh <cmd>: …`, `console.EXIT_*`) · `console.emit_json` (raw ints) · `fsio.append_jsonl` — the one-way-per-thing canon (EP-33 B8, [`docs/gotchas.md`](../mimicwarehouse/docs/gotchas.md)). |
...
**Sizes** (D-2): S ≈ 30 min, M ≈ 1 h, L ≈ 2 h of one Claude session with the owner supervising; anything larger is split at pickup. Current mix: 21 S · 142 M · 1 L ≈ 155 h total, ≈ 145 h core (the 11 stretch briefs are the P10 text track, EP-123 and EP-145) — roughly three months at 12 h/week, four at 9 h/week. *(EP-7, 2026-08-17: EP-164 (S, core) allocated into P1 → 165 briefs, 22 S; P0's seven briefs actually took ≈ 3 h 20 min against ≈ 4 h 30 min planned — see the EP-7 retro table.)* *(Retro, 2026-08-18: six retro briefs EP-165 … EP-170 (5 M + 1 S, core) allocated — five into P1 after EP-12, one at the head of P2 — from the owner-directed retrospective review of EP-0 … EP-12 → 171 briefs, 23 S · 147 M · 1 L; the review's findings ledger is [`retro-2026-08-18-findings.md`](retro-2026-08-18-findings.md), the decisions are D-43.)* *(P1 re-plan follow-up, 2026-08-29: the owner allocated EP-171 (S, core, write canary) into P2 before EP-17 → 172 briefs, 24 S · 147 M · 1 L.)* *(Owner directive, 2026-08-30 — D-44: EP-33 re-scoped S → L as the single-session consolidation re-plan of P0–P2 (see its amendment block) → 172 briefs, 23 S · 147 M · 2 L; it runs as one owner-supervised multi-agent session, so D-2's split-at-pickup rule is deliberately waived for it.)* *(Owner directive, 2026-09-16 at EP-45 completion: EP-172 (M, core, the GEM review adjudication) allocated into P3 between EP-45 and EP-46 → 173 briefs, 23 S · 148 M · 2 L.)*
...
| EP-167 | [Retro C: CLI, settings & inventory consolidation](EP-167-retro-cli-config-consolidation.md) | M | EP-3, EP-10 | core | ☑ `b08ef56` |
| EP-168 | [Retro D: test-infrastructure consolidation (tier readiness, demo marker, churn)](EP-168-retro-test-infrastructure.md) | S | EP-12 | core | ☑ `a0356c0` |
| EP-169 | [Retro E: contract tie-breaks, structural hash & fixture regeneration (0.2.0)](EP-169-retro-contract-fixture-regen.md) | M | EP-9, EP-12 | core | ☑ `35fd386` |
| EP-13 | [Repos & awesome-lists inventory](EP-13-resources-repos.md) | M | — | core | ☑ `d285522` |
| EP-14 | [Ontologies & vocabularies inventory](EP-14-resources-vocabularies.md) | M | — | core | ☑ `0ed9516` |
| EP-15 | [Reading list + companion datasets + methods notes](EP-15-resources-reading-datasets.md) | M | — | core | ☑ `3e2ff6d` |
| EP-16 | [Re-plan P1](EP-16-replan-p1.md) | S | EP-8, EP-9, EP-10, EP-11, EP-12, EP-13, EP-14, EP-15 | core | ☑ `483c99d` |

Ordering rationale: EP-8 (vendor mimic-code) precedes EP-9 (schema contract is transcribed from `create.sql`) precedes EP-10 (raw inventory reconciles against `validate.sql`); fixtures (EP-11/12) come after the schema contract they must satisfy. EP-13..15 are independent and may run in any order. **EP-164** (added 2026-08-17 at the P0 re-plan, EP-7; owner decision, Risk 12 / D-38 addendum) is the P1 toolchain-remediation slot — a `mwh doctor` `antivirus` check — and runs first so the doctor names both endpoint-security products before EP-10 hashes 98 GB of CSVs and EP-11/12 write fixture trees. **EP-165 … EP-169** (added 2026-08-18 by the retrospective review of EP-0 … EP-12; owner decisions D-43; findings ledger `retro-2026-08-18-findings.md`) sit between EP-12 and EP-13 so that (a) every later session — including the three docs briefs — starts from a CLAUDE.md that carries the D-42/PATH/console rules and a hardened deny/allow/hook layer (EP-165), (b) the status prose a cold session reads first is true (EP-166), and (c) the CLI/config, test-infrastructure and contract/fixture consolidations (EP-167 … EP-169) land **before** EP-16 verifies P1 and before any P2 brief codes against them; **EP-170** (head of P2) then reconciles the P2/P3 briefs with the shipped names, replacing EP-16 item 6. Each is a deliberate S/M so a single session can finish and commit it.
...
| EP-30 | [Safe-query wrapper + audit log](EP-30-safe-query-audit.md) | M | EP-21 | core | ☑ `5566308` |
| EP-31 | [Tracer bullet: first-ICU-stay adults → in-hospital mortality](EP-31-tracer-bullet.md) | M | EP-21, EP-30 | core | ☑ `0252061` |
| EP-32 | [Capstone #0: staging benchmark note + docs/analyses convention](EP-32-capstone-0-staging.md) | S | EP-28, EP-31 | core | ☑ `e832218` |
| EP-33 | [Re-plan P2](EP-33-replan-p2.md) | L | EP-17, EP-18, EP-19, EP-20, EP-21, EP-22, EP-23, EP-24, EP-25, EP-26, EP-27, EP-28, EP-29, EP-30, EP-31, EP-32 | core | ☑ `876e781` + `d88e2b5` + `2aa9e65` + `84d9d8d` + `8848534` + `00a7e50` + `3e2f872` + `1d6abcf` |

Ordering rationale: Loader (EP-17/18) → DAG runner (EP-19) → small tables (EP-20) → catalog (EP-21) → demo tier (EP-22) so the tracer bullet (EP-31) can run before the ⏱ chartevents pass finishes; EP-23 (labevents) is the dress rehearsal for EP-26 (chartevents); ⏱ briefs launch background jobs verified by EP-28; safe-query (EP-30) precedes the tracer because every result the tracer shows must pass through it. **EP-171** (added 2026-08-29 at the P1 re-plan follow-up; owner decision, superseding EP-16's "not needed" recommendation) is the P2 toolchain-remediation slot — a synthetic write canary rehearsing the loader's burst/large/manifest/swap/delete I/O shapes against the Malwarebytes ARW heuristic (Risk 12) and measuring the write-side MB/s baseline — and runs after EP-170, before EP-17, so the pattern is proven before any staging code writes into the data root. **EP-33** was re-scoped by the owner on 2026-08-30 (D-44) from the standard S re-plan to an L, single-session **consolidation re-plan of P0–P2** — the standing re-plan duties plus a known-debt fix/paradigm-unification pass, a DESIGN as-built consolidation, prospective P3 hardening and a bounded discovery audit — executed as one supervised multi-agent ("ultracode") session before EP-34 opens P3; its amendment block carries the charter.
...
| EP-36 | [Seed/determinism policy + resource logger](EP-36-seeds-resource-log.md) | S | EP-35 | core | ☑ `8261089` |
...
The full record is [`../mimicwarehouse/DECISIONS.md`](../mimicwarehouse/DECISIONS.md) (D-1 … D-43 + assumed defaults + judgment calls). The twelve that shape sequencing most:
...
1. ~~Plain CSVs cannot be verified against PhysioNet's `SHA256SUMS.txt` (covers `.csv.gz` only)~~ **Resolved by EP-10 (2026-08-17)** — the local manifest is the raw snapshot id (D-26); `.csv.gz` re-download parked in `final-roadmap.md` (RAW-1): manifest complete for all 41 files (104,641,868,093 bytes, 902,815,672 rows; the two passes took ≈ 93 s in total at 2.0–2.4 GB/s, not the 10–30 min planned), every header equals the EP-9 contract, every upstream `validate.sql` count matches (34 match · 0 mismatch · 7 without expectation), `raw_snapshot_id 8209301d8a06…` — see `mimicwarehouse/docs/resources/raw-inventory.md` and the D-26 addendum. The `.csv.gz` archive hashes are carried per file so RAW-1 can re-verify later.
2. `concepts_duckdb` lags `concepts/` upstream (open regeneration PR; open concept-logic PRs: SIRS wbc guard, lab valueuom, Charlson, APS-III) and its README targets DuckDB 1.4 LTS → EP-37/38 pin, count-pin and port; no ED/Note concepts exist upstream (ours). **Executability half resolved by EP-33 (2026-08-30)** — the D2 pre-flight smoke executed all 65 vendored `concepts_duckdb` files cleanly in driver order on DuckDB 1.5.5 against a throwaway demo-catalog copy (1.78 s, zero failures of any kind), so EP-38's charter shifts from "fix recorded breakage" to count-pin verification and upstream-PR ports; the upstream-lag/PR half stays open. **Upstream-PR half ported by EP-38 (2026-09-06)** — the four open concept-logic PRs are patches over the untouched vendored files (`concepts/patches/`, status `ported-unmerged`: SIRS `wbc_max` guard #2146, MCHC / CRP `valueuom` filters #2141, Charlson C4A exclusion #2142, APS III equidistant arms #2137; no demo count-pin moved), the registry is pinned to the vendored commit so a re-vendor refuses every concept build until each patch is reviewed, and the full derived layer was verified (65 / 65, 95,777,751 rows). Still open: the upstream lag itself (the PRs and the regeneration PR #2157 are unmerged) — re-check at EP-54 / EP-74 and drop each patch once its PR lands in the pin.
...
6. Disk: 98 GB CSV + ~~≈20 GB lake + 60–100 GB build-temp peak (staging)~~ **Staging portion resolved by EP-28 (2026-08-29)** — core lake measured **7.0 GB** (7,046,156,578 bytes; 13.8× compression, DESIGN §3 note) and no build-temp spill was observed in any ⏱ staging job; **392.8 GB free** after P2 staging; ~~still open: 15–30 GB derived/spine + 5–15 GB notes/embeddings + models (temp behaviour of the concept/spine builds re-estimated at EP-33)~~ **Derived/spine/marts half re-estimated by EP-33 (2026-09-01, item D3)** — from the measured 13.8× compression and a demo-scale run of all 65 vendored concepts (DESIGN §3 note): derived + spine ≈ **4–6 GB** (was 15–30), marts ≈ **1–2 GB** (was 5–15), concept/spine build-temp spill possible but bounded (≤ 20–40 GB worst case, 0 expected for most concepts); EP-37/EP-50's ⏱ jobs measure, EP-54 records; notes/embeddings (5–15 GB, P10) + models + ~15 GB uv/venv + hiberfil/pagefile unchanged → never below 100 GB free (`mwh doctor`/`mwh build` refuse — the floor stays enforced).
...
8. Governance: Claude Code ships tool results to Anthropic → k-suppressed aggregates only, never note text (GOVERNANCE §4); git history is permanent → guard/gitignore/gitattributes before any data code (EP-0/EP-4); MIMIC-IV-Note is a separate DUA and the highest-risk asset; ~~owner supplies CITI + DUA acceptance dates for GOVERNANCE §1 and checks the claude.ai training toggle~~ **Resolved by the EP-0 follow-up (2026-08-17)** — GOVERNANCE §1 filled, toggle confirmed off (D-38 addendum); suspected PHI → PhysioNet only. EP-0 (2026-08-17) verified the `.claude/settings.json` deny rules refuse a synthetic `*.csv` inside the repo (Read/Bash/PowerShell) but `Read(**/*.csv)` is project-relative and did not fire on the same probe under `%TEMP%`; `C:\mimicdata` relies on the explicit `//C:/mimicdata/**` rules — the EP-4 guard and EP-30 safe-query remain necessary layers. Since **EP-165** (2026-08-28) the session layer is denies **plus** an allow list and a live **PreToolUse command-string hook** (commands/paths mentioning `mimicdata`/`source material`/`.csv`/`.parquet`/`.duckdb` refused outside allow-listed project launchers; absolute `Read(//C:/**/*.csv…)` rules close the `%TEMP%` gap), and claude.ai **connectors/WebFetch are a governed second egress path** — public-reference lookups only, send/write tools denied (GOVERNANCE §4, D-39/D-43).
...
11. Time budget: ≈ 161 h for 173 briefs (23 S · 148 M · 2 L since EP-172; was ≈ 155 h for 164 at planning); if the owner's cadence is below ~10 h/week, the P7 re-plan should move P10 and further stretch items across the cutline rather than compress briefs.
12. **Endpoint security is two real-time products** (Windows Defender + Malwarebytes 5.1 Premium), both on, the toolchain and both data locations allow-listed (nine Malwarebytes paths since D-43); sessions adapt their I/O pattern — no heredocs, no burst file loops, resumable writers — and triage any "process killed / binary vanished" symptom against the Malwarebytes quarantine first. Canonical record: **D-42** + the D-38 addenda in `DECISIONS.md`; session rules in `CLAUDE.md` §3; `mwh doctor` `antivirus` (EP-164) warns by design on this host. **→ EP-171 write canary passed (2026-08-29):** the loader's five write shapes (Parquet burst / large sequential / manifest churn / rename-aside swap / delete loop) rehearsed with synthetic bytes under `C:\mimicdata\tmp\canary` — process survived, every sha256 re-read matched, nothing quarantined; write baselines 191 MB/s (burst small-file) and 239 MB/s (large sequential, DuckDB defaults) in the EP-171 completion note. *(Narrative trimmed at EP-166 — D-43 item 13/DOC-11; the incident history lives in the D-38 EP-6→EP-7 addendum and the EP-7/EP-164 completion notes.)*
13. **Console code page (cp1252) vs rich/Unicode CLI output.** `PYTHONUTF8=1` ships to the tool shells via `.claude/settings.json` since EP-165; new CLI strings still stay ASCII or pass through the shared console helper (EP-167, D-43 item 12), because hosts outside the Claude shells may remain cp1252. Canonical record: DESIGN §2 console notes (EP-7 + EP-166). *(Trimmed at EP-166 — D-43 item 13.)*
...
15. **Retrospective review of EP-0 … EP-12 (2026-08-18).** An owner-directed adversarial review (ten lenses → one verifier per material finding → completeness critic; 81 agents, nothing implemented in the review itself) recorded **69 verified + 110 minor findings** in [`retro-2026-08-18-findings.md`](retro-2026-08-18-findings.md). None is a data-safety incident; the material ones are: CLAUDE.md never absorbed D-42 / the uv-PATH and cp1252 realities; the `.claude` deny list is a prefix blocklist that `python -c` / `uv run python` / `cp` to the scratchpad / nested `sh -c` bypass (owner: extend + unpark the PreToolUse hook); MCP connectors are an undocumented egress path; the Windows catalog swap is not atomic while READ_ONLY readers are open (owner: rename-aside protocol); fixture/demo tiers had no lake root; the pytest dev gate would make EP-17 … EP-20 acceptance vacuous; contract sort keys / `content_hash()` / fixture id floors would each force repeated fixture regenerations; status docs frozen at EP-1; several P2/P3 brief assumptions already wrong. Owner decisions are **D-43**; the fixes are the six retro briefs **EP-165 … EP-170** (each S/M). Until they land, sessions must read the ledger's index for their brief's ids. → *struck per brief as each lands.* → **EP-165 done (2026-08-28, `b9b8d03`)** — settings env/deny/allow, PreToolUse hook live, CLAUDE.md §3, GOVERNANCE §2/§4, `.gitignore` anchoring, guard G1/G4. → **EP-166 done (2026-08-28)** — status surface (workspace README § State), DESIGN §5/§6/§11 protocols + identifier glossary, D-43 distributed into DECISIONS addenda + owner verdicts, this file's notation table and Risk trims. → **EP-167 … EP-171 done (2026-08-28/29)**; the 2026-08-18 ledger's 69 findings were re-verified at EP-33 (66 held, 3 partial — FXT-4 vocab regen → EP-41 **(done 2026-09-06: generator 0.3.0)**, CFG-2 remainder closed by EP-33 B3, FC-9 `vl-convert-python` placement → EP-54).

16. **EP-33 consolidation of P0–P2 (2026-09-01, D-44/D-45).** A second bounded audit (13 lenses → verifier per finding; 75 agents) recorded **46 verified + 60 carried-low findings** in [`retro-p2-findings.md`](retro-p2-findings.md); the owner triaged them at one checkpoint (36 fix-now, all landed in-session; 4 rejected with reasons; 2 to the session-guard package; the carried-lows swept where touched or parked as `final-roadmap.md` AUDIT-1 for EP-54). Headline defects fixed: a forced dev-tier restage silently discarding a full table's other 95 partitions (LDR-1); an alias-only count rule that let below-k groups through the safe-query gate (DKB-1/SGT-1); a swallowed `FileNotFoundError` that could delete a table's only live copy during a quarantined publish (CLI-1/LDR-2); a crash window that dropped manifest lines (WIN-1/LDR-3). → **B7 package applied by the owner (2026-09-01, `1d6abcf`)** — SGD-1/SGD-2 closed; the fixtures Read allowance stays not proposed (D-45 item 4). Record: [`retro-p2.md`](retro-p2.md), D-45.

17. **Monotone count sequences in published tables (EP-45, 2026-09-16).** `meta.mp_item_hourly.n_stays_at_risk` (stays still in the ICU per hour bin) is a non-increasing sequence, so the difference of two published neighbours — the stays leaving the ICU in one bin — is a derivable count that can fall below k; `disclose.suppress` (table mode) hides cells, not steps, the chain mode's 10-banding maps 11–14 to 10 (below k), and `mwh disclose check` does not treat an at-risk curve as an attrition chain. Recorded in the EP-45 D-33 addendum and parked as `final-roadmap.md` MISS-4; the owner re-decides before EP-53 promotes the report or EP-72 renders the views (the same shape recurs in any survival-style at-risk table: EP-91's KM tables).

18. **Partitioned `COPY` seams in the loader's small-path files (EP-50, 2026-09-17).** On DuckDB 1.5.5 under the build profile's 12 threads a sorted `COPY … PARTITION_BY` combines per-thread buffers, so a partition file can carry one seam where the `(subject_id, …)` order restarts — regardless of `preserve_insertion_order` (a 3 M-row synthetic probe reproduces it with the setting on and off; a single-file `COPY … ORDER BY` is sorted in both). The EP-50 spine therefore writes one sorted file per bucket (the pass-2 shape) and its `sorted_within_file` check passes on dev and full; the loader's *small* path (DESIGN §5: "each partition file lands sorted") carries the same seams in the dev-tier core files of `diagnoses_icd` (2) and `outputevents` (1), the other small tables clean, the large path (pass 2, per-bucket sorts) unaffected. No reader depends on physical order and the sha256 determinism tests still hold, so nothing is broken today; EP-54 decides between document-and-leave, an in-place per-bucket re-sort, and switching the small path to per-bucket `COPY`s + a restage (`final-roadmap.md` LOAD-5; `docs/gotchas.md` §1).
