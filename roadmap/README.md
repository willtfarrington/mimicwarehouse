...
**Sizes** (D-2): S ≈ 30 min, M ≈ 1 h, L ≈ 2 h of one Claude session with the owner supervising; anything larger is split at pickup. Current mix: 21 S · 142 M · 1 L ≈ 155 h total, ≈ 145 h core (the 11 stretch briefs are the P10 text track, EP-123 and EP-145) — roughly three months at 12 h/week, four at 9 h/week.
...
|---|-------|------|-----------|------|------|
| EP-0 | [Baseline & hygiene](EP-0-baseline.md) | S | — | core | ☐ |
| EP-1 | [Toolchain bootstrap (uv + CPython 3.13 + pyproject)](EP-1-toolchain-bootstrap.md) | M | EP-0 | core | ☐ |
| EP-2 | [`mwh` CLI skeleton + `mwh doctor`](EP-2-mwh-cli-doctor.md) | S | EP-1 | core | ☐ |
...
Ordering rationale: EP-8 (vendor mimic-code) precedes EP-9 (schema contract is transcribed from `create.sql`) precedes EP-10 (raw inventory reconciles against `validate.sql`); fixtures (EP-11/12) come after the schema contract they must satisfy. EP-13..15 are independent and may run in any order.
...
3. Resolver traps: Streamlit `pyarrow<25` (→ `ui` group, EP-1/EP-57); pygam `scipy<1.17` (→ statsmodels GAM); scikit-survival pins `sklearn==1.9` and is GPL-3 (→ `gpl` group); econml `<1.10`; pytensor `numba<=0.66`; pandas 3.0 str dtype/CoW/µs datetimes → EP-1 smoke test.
4. Windows: MAX_PATH (owner enables LongPathsEnabled; short data root); `spawn` multiprocessing (`__main__` guards; no module-level connections/GPU init); uv cache and `.venv` on the same volume; CRLF (`.gitattributes`); Defender exclusion for the data root only (owner); foreground shell cap ~10 min → background jobs with logs; laptop thermal throttling → log clocks.
